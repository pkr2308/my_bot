"""
RP2040 Firmware - Fixed version for reliable PING response
"""
...
def handle_input():
    try:
        ...
        # For compatibility when select is not available
        try:
            line = sys.stdin.readline()
            if line:
                response = controller.process_command(line)
...
# Import select if available, otherwise use simpler version
try:
    import select
    has_select = True
except ImportError:
...
# Modified main loop for better compatibility
if __name__ == "__main__":
    print("RP2040 Controller ready")
    
    if has_select:
        # Non-blocking version with select
        while True:
            try:
                handle_input()
        ...
        # Blocking version without select
        while True:
            try:
                handle_input()
            except KeyboardInterrupt:
    ...
    print("RP2040 Controller loaded - call handle_input() for commands")
